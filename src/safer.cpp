...
namespace Safer {
    namespace {
        inline uint8_t rotl (uint8_t val, size_t count) {
         ...
         *                [1 1]
         */
        inline void pht (uint8_t& x, uint8_t& y) {
            y = x + y;
            x = x + y;
        }
         ...
         *                [-1  2]
         */
        inline void ipht (uint8_t& x, uint8_t& y) {
            x = x - y;
            y = y - x;
        }

        void BuildKeyBlock (block_t result[2], const void* key, size_t key_size) {
    ...
    Key::Key (const Key &src) noexcept
            : nRounds_ (src.nRounds_) {
        std::memcpy (values_, src.values_, sizeof (values_)) ;
    }

    Key &       Key::Assign (const Key &src) noexcept {
        nRounds_ = src.nRounds_ ;
        std::memcpy (values_, src.values_, sizeof (values_)) ;
        return *this ;
    }
